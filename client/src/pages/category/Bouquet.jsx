...
export default function ShopPage() {
  // ✅ Using bouquet-specific data from ProductContext
  const { bouquets, loading, fetchAllProducts } = useProducts();
  ...
  const handleSort = async (value) => {
    ...
    if (value === "low-high") {
      await fetchAllProducts("?sort=price_asc&category=bouquet");
    } 
    else if (value === "high-low") {
      await fetchAllProducts("?sort=price_desc&category=bouquet");
    } 
    else if (value === "new-old") {
      await fetchAllProducts("?sort=newest&category=bouquet");
    }
    else if (value === "old-new") {
      await fetchAllProducts("?sort=oldest&category=bouquet");
    } 
    else {
      return;
    }
  ...
  return (
    <section className="max-w-[1280px] mx-auto px-4 md:px-6 py-10 overflow-x-hidden">
      ...
      <div className="flex items-center justify-between mb-8">
        <h1 className="text-xl font-semibold">Bouquets</h1>
      ...
      {loading ? (
        ...
        /* ✅ Show bouquets instead of all products */
        <div className="grid sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 md:gap-6">
          {bouquets.map((item) => (
