...
export default function ShopPage() {
  ...
  let { gifts, loading, fetchAllProducts } = useProducts();
  let bouquets = gifts
  ...
  const handleSort = async (value) => {
    ...
    if (value === "low-high") {
      await fetchAllProducts("?sort=price_asc&category=gift");
    } 
    else if (value === "high-low") {
      await fetchAllProducts("?sort=price_desc&category=gift");
    } 
    else if (value === "new-old") {
      await fetchAllProducts("?sort=newest&category=gift");
    }
    else if (value === "old-new") {
      await fetchAllProducts("?sort=oldest&category=gift");
    } 
    else {
      return;
    }
  ...
  return (
    <section className="max-w-[1280px] mx-auto px-4 md:px-6 py-10 overflow-x-hidden">
      ...
      <div className="flex items-center justify-between mb-8">
        <h1 className="text-xl font-semibold">Gift Sets</h1>
